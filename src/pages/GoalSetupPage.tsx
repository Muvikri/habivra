    ...
    return (
      <div className="flex-1 flex flex-col justify-between p-6">
        <div className="space-y-6">
          <PageHeader
            title="Buat Profilmu"
            subtitle="Langkah 1 dari 2 · Kenalkan dirimu ke Habivra"
            showBack
            backPath="/onboarding"
          />
          <div className="flex flex-col items-center gap-3 pt-3">
            <button
              ...
              className="relative w-28 h-28 rounded-3xl overflow-hidden bg-[var(--accent-muted)] border-2 border-[var(--border-default)] flex items-center justify-center text-[var(--accent-primary)]"
            >
              {avatarPreview ? (
                <img
                  src={avatarPreview}
            ...
            <p className="text-xs font-semibold text-[var(--text-muted)]">
              Tambahkan foto profil (opsional, maks. 2 MB)
            </p>
          </div>
          <div>
            <label className="block text-xs font-extrabold text-[var(--text-primary)] mb-2">
              Nama tampilan
            </label>
            <input
              value={name}
              ...
              className="w-full px-4 py-3.5 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-default)] text-sm font-bold text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)]"
            />
          </div>
          {errorMessage && (
        ...
        <button
          ...
          disabled={saving}
          className="w-full py-4 rounded-2xl bg-[var(--accent-primary)] text-white text-xs font-black transition-all shadow-md active:scale-95 disabled:opacity-50 flex justify-center items-center gap-2"
        >
          {saving ? "Menyimpan..." : "Lanjut ke Tujuan"}{" "}
    ...
    <div className="flex-1 flex flex-col justify-between p-6">
      <div>
        <PageHeader
          ...
          subtitle="Langkah 2 dari 2 · Pilih satu atau lebih fokus kebiasaan hijau"
        />
        <button
          onClick={() => setStep("profile")}
          className="mt-4 text-xs font-bold text-[var(--accent-primary)]"
        >
          ← Edit profil
        </button>
