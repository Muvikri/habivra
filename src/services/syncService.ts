...
async function applyOperation(operation: QueueOperation) {
  if (operation.type === 'habit.create') return supabase.from('habits').insert(operation.habit)
  if (operation.type === 'habit.toggle') return supabase.from('habits').update({ done: operation.done, updated_at: new Date().toISOString() }).eq('id', operation.id)
...
export const syncService = {
  async flush(userId: string) {
    ...
    for (const operation of queue) {
      const { error } = await applyOperation(operation)
